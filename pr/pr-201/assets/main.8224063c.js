 ...
 * Copyright 2021 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const bs=(r,...e)=>({strTag:!0,strings:r,values:e}),ht=bs,ys=r=>typeof r!="string"&&"strTag"in r,ws=(r,e,t)=>{let i=r[0];for(let a=1;a<r.length;a++)i+=e[t?t[a-1]:a-1],i+=r[a];return i};/**
 * @license
 * Copyright 2021 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
...
`;let ca=class extends A{constructor(){super(...arguments),this.config=new Kr}render(){return p`
    ...
    `}static get styles(){const e=f`var(--modalLogoSize, 6.5rem)`,t=f`var(--processingImageSize, 7.5rem)`,i=f`var(--modalCornerRadius, 1rem)`,a=f`var(--modalBorder, 2px solid black)`,o=f`var(--modalBottomMargin, 2.5rem)`,n=f`var(--modalTopMargin, 5rem)`,l=f`var(--modalHeaderBottomPadding, 0.5em)`,d=f`var(--modalBottomPadding, 2rem)`,u=f`var(--modalScrollOffset, 5px)`,m=f`var(--modalTitleFontSize, 1.8rem)`,v=f`var(--modalSubtitleFontSize, 1.4rem)`,b=f`var(--modalHeadlineFontSize, 1.6rem)`,x=f`var(--modalMessageFontSize, 1.4rem)`,$=f`var(--modalTitleLineHeight, normal)`,M=f`var(--modalSubtitleLineHeight, normal)`,I=f`var(--modalHeadlineLineHeight, normal)`,H=f`var(--modalMessageLineHeight, normal)`;return f`
      .processing-logo {
        margin: auto;
        width: ${t};
        height: ${t};
