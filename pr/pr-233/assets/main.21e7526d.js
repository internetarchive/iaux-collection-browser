...
`;let mo=class extends D{constructor(){super(...arguments),this.config=new ir}render(){return h`
    ...
    `}static get styles(){const e=m`var(--modalLogoSize, 6.5rem)`,t=m`var(--processingImageSize, 7.5rem)`,i=m`var(--modalCornerRadius, 1rem)`,o=m`var(--modalBorder, 2px solid black)`,r=m`var(--modalBottomMargin, 2.5rem)`,n=m`var(--modalTopMargin, 5rem)`,l=m`var(--modalHeaderBottomPadding, 0.5em)`,d=m`var(--modalBottomPadding, 2rem)`,u=m`var(--modalScrollOffset, 5px)`,v=m`var(--modalTitleFontSize, 1.8rem)`,f=m`var(--modalSubtitleFontSize, 1.4rem)`,b=m`var(--modalHeadlineFontSize, 1.6rem)`,x=m`var(--modalMessageFontSize, 1.4rem)`,_=m`var(--modalTitleLineHeight, normal)`,k=m`var(--modalSubtitleLineHeight, normal)`,E=m`var(--modalHeadlineLineHeight, normal)`,B=m`var(--modalMessageLineHeight, normal)`;return m`
      .processing-logo {
        margin: auto;
        width: ${t};
        height: ${t};
