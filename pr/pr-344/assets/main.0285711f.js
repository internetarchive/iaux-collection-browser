...
`;var Vo;(function(r){r[r.bytes=0]="bytes",r[r.kilobytes=1]="kilobytes",r[r.megabytes=2]="megabytes",r[r.gigabytes=3]="gigabytes",r[r.terabytes=4]="terabytes",r[r.petabytes=5]="petabytes",r[r.exabytes=6]="exabytes",r[r.zettabytes=7]="zettabytes",r[r.yottabytes=8]="yottabytes"})(Vo||(Vo={}));function jl(r,e,t=" "){let i=r;if(i===void 0)return y;let o=0;for(;i>1024;)i/=1024,o+=1;const a=10**e;i=Math.round(i*a)/a;let s=Vo[o];return s=i===1?s.slice(0,-1):s,`${i.toLocaleString()+t+s}`}const Gl=g`var(--tileBackgroundColor, #ffffff)`,ql=g`var(--tileCornerRadius, 4px)`,Qi=g`
 ...
 * Copyright 2021 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */var zo;((zo=window.HTMLSlotElement)===null||zo===void 0?void 0:zo.prototype.assignedElements)!=null;var vd=Es`<svg class="caret-up-svg" viewBox="0 0 8 4" xmlns="http://www.w3.org/2000/svg">
...
`,lr;let It=lr=class extends F{render(){return c`${this.facetRowTemplate}`}get facetRowTemplate(){var e,t,i,o;const{bucket:a,facetType:s}=this;if(!a||!s)return y;const n=`${s}:${a.key}-show-only`,d=`${s}:${a.key}-negative`,h=s!=="collection"?c`${(e=a.displayText)!==null&&e!==void 0?e:a.key}`:c`<a href="/details/${a.key}">
    ...
    `}get loaderTemplate(){return c`<div class="facets-loader">
      <ia-activity-indicator .mode=${"processing"}></ia-activity-indicator>
        ...
        </div> `:y}sortFacetAggregation(e){this.sortedBy=e,this.dispatchEvent(new CustomEvent("sortedFacets",{detail:this.sortedBy}))}get getModalHeaderTemplate(){const t=No[this.facetKey]===ee.COUNT?"left":"right";return c`<span class="sr-only">More facets for:</span>
      <span class="title">
    ...
    `}searchMoreFacetsLink(e){if(!this.moreLinksVisible||e.key==="lending"||Object.keys(e.buckets).length<this.allowedFacetCount)return y;const t=No[e.key];return c`<button
      class="more-link"
